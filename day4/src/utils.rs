...
impl Area{
    fn contains(&self, other: &Self) -> bool{
        let l_start = if self.start < other.start {true} else {false};
...
pub fn get_overlap_pairs(input: &str) -> AnyResult<u32>{
    ...
    let count: u32 = areas.into_iter().map(|pair| if (pair.0).overlap(&(pair.1)) {1} else {0}).sum();
    Ok(count)
}

pub fn get_containing_pairs(input: &str) -> AnyResult<u32> {
    let areas = get_areas(input)?;
    let count: u32 = areas.into_iter().map(|pair| if (pair.0).contains(&(pair.1)) {1} else {0}).sum();
    Ok(count)
}

fn get_areas(input: &str) -> AnyResult<Vec<(Area,Area)>>{
    let input_1 = fs::read_to_string(input)?;
